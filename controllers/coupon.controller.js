...
export const createCoupon = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.role === "creator") {
    // Creators must always target specific courses — never "all"
    if (!applicableTo || applicableTo === "all")
      return next(new AppError("Creators must select specific courses for a coupon.", 403));

    if (!courses || courses.length === 0)
      return next(new AppError("Select at least one course.", 400));

    // Verify every selected course is owned by this creator
    const owned = await Course.find({
      _id: { $in: courses },
      creator: req.user._id,
    }).select("_id");

    if (owned.length !== courses.length)
      return next(new AppError("You can only create coupons for your own courses.", 403));
  }
  ...
  const coupon = await Coupon.create({
    ...
    // Creators are always locked to specific_courses regardless of what was sent
    applicableTo: req.user.role === "creator" ? "specific_courses" : (applicableTo || "all"),
